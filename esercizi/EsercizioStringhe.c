...
int stringsplit(long **values, const char *s) {
    ...
    *values = (long *)malloc(count * sizeof(long));
    ...
    while (*start != '\0') {
        ...
        long num = strtol(start, &end, 10); //prende i valori da start ad end(ferma alla virgola) in base 10
...
int main() {
    ...
    fgets(input, sizeof(input), stdin); //legge i valori messi dall'utente
